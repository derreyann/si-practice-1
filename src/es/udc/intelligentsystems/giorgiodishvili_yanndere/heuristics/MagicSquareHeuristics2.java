...
/**
 * was not really helpful of improvement
 */
public class MagicSquareHeuristics2 extends Heuristic {
    /**
    ...
    public float evaluate(State e) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            if (rowSum[i] != board.getSum()) {
                if (rowSum[i] + board.getBoardLastNumber() < board.getSum()) {
                    boardSum += board.getSum() - rowSum[i];
                } else {
                    boardSum += rowSum[i];
                }
            }
            if (colSum[i] != board.getSum()) {
                if (colSum[i] + board.getBoardLastNumber() < board.getSum()) {
                    boardSum += board.getSum() - colSum[i];
                } else {
                    boardSum += colSum[i];
                }
//                boardSum += colSum[i];
        ...
        if (diagonal + board.getBoardLastNumber() < board.getSum()) {
            boardSum += board.getSum() - diagonal;
        } else {
            boardSum += diagonal;
        }
        // sum check for reverse diagonal
        if (reverseDiagonal + board.getBoardLastNumber() < board.getSum()) {
            boardSum += board.getSum() - reverseDiagonal;
        } else {
